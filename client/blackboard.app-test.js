...
describe('blackboard', function() {
  ...
  it('sorts rounds in requested order', async function() {
    ...
    await afterFlushPromise();
    return chai.assert.isBelow($(`#round${civ._id}`).offset().top, $(`#round${emo._id}`).offset().top);
  });
  ...
  it('navigates to puzzle on click', async function() {
    ...
    return chai.assert.equal(Session.get('id'), isss._id);
  });
  ...
  it('hides solved', async function() {
    ...
    return reactiveLocalStorage.setItem('hideSolved', 'false');
  });
  ...
  describe('presence filter', function() {
    ...
    before(async function() {
      ...
      const p1 = new Promise(resolve => other_conn.subscribe('register-presence', `puzzles/${puzz1._id}`, 'chat', {onReady: resolve}));
      const p2 = new Promise(resolve => other_conn.subscribe('register-presence', `puzzles/${puzz2._id}`, 'jitsi', {onReady: resolve}));
      await Promise.all([p1,p2]);
      Router.BlackboardPage();
      await waitForSubscriptions();
      await afterFlushPromise();
      return $('.bb-show-filter-by-user').click();
    });
    ...
    const checkPage = function() {
      ...
      return chai.assert.isNotOk($(`[data-puzzle-id=\"${Puzzles.findOne({name: 'AKA'})}\"]`)[0]);
    };
    ...
    return after(() => other_conn.disconnect());
  });
  ...
  describe('in edit mode', function() {
    ...
    it('allows reordering puzzles', async function() {
      ...
      const mathsJQ = $(`#m${wall_street._id} tr[data-puzzle-id=\"${maths._id}\"]`);
      const cheatersJQ = $(`#m${wall_street._id} tr[data-puzzle-id=\"${cheaters._id}\"]`);
      ...
      await waitForMethods();
      await afterFlushPromise();
      return chai.assert.isBelow(mathsJQ.offset().top, cheatersJQ.offset().top, 'after up');
    });
    ...
    it('alphabetizes within a meta', async function() {
      ...
      const cluelessJQ =  disgustJQ.find(`tr[data-puzzle-id=\"${clueless._id}\"]`);
      ...
      await waitForMethods();
      await afterFlushPromise();
      return chai.assert.isBelow(cluelessJQ.offset().top, akaJQ.offset().top, 'after manual');
    });
    ...
    it('allows creating and deleting puzzles with buttons', async function() {
      ...
      const indirectTitle = $(`#m${meta._id} tr.puzzle[data-puzzle-id=\"${indirect._id}\"] .bb-puzzle-title`);
      indirectTitle.click();
      await afterFlushPromise();
      ...
      await waitForMethods();
      return chai.assert.isNotOk(Puzzles.findOne(indirect._id));
    });
    ...
    it('adds and deletes tags', async function() {
      ...
      const baseJq = $(`tbody.meta[data-puzzle-id=\"${initial.feedsInto[1]}\"] [data-puzzle-id=\"${initial._id}\"]`);
      ...
      await waitForMethods();
      const deleted = bank();
      return chai.assert.notOk(deleted.tags.meme);
    });
    ...
    it('renames tag', async function() {
      ...
      await waitForMethods();
      return chai.assert.isNotOk(disgust.tags.color5);
    });
    ...
    it('empty name aborts', async function() {
      ...
      await waitForMethods();
      disgust = Puzzles.findOne(disgust._id);
      return chai.assert.isOk(disgust.tags.color3);
    });
    ...
    return it('will not clobber a tag', async function() {
      ...
      await waitForMethods();
      disgust = Puzzles.findOne(disgust._id);
      return chai.assert.isOk(disgust.tags.color2);
    });
  });
  ...
  return it('makes a puzzle a favorite', async function() {
    ...
    return chai.assert.isDefined($(`tr[data-puzzle-id=\"${bank._id}\"] .bb-recent-puzzle-chat`).html());
  });
});
