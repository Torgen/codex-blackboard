...
describe('chunk_text', function() {
  it('inserts newline before mention', () => chai.assert.deepEqual(chunk_text('@foo: do x\n@bar: do y'), [
  ...
  it('supports mention characters', () => chai.assert.deepEqual(chunk_text('@test_1x: yo'), [
    {type: 'mention', content: 'test_1x'},
    {type: 'text', content: ': yo'}
  ]));

  return it('matches urls without protocol', () => chai.assert.deepEqual(chunk_text('it\'s www.foo.com/bar, yo'), [
...
describe('chunk_html', () => it('processes text outside tags', () => chai.assert.deepEqual(chunk_html('@torgen: there\'s already <i class="fas fa-link"></i><a href="foo">a puzzle named bar</a>.'), [
