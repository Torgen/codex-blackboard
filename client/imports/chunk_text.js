const urlRE = /\b(?:[a-z][\w\-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]|\((?:[^\s()<>]|(?:\([^\s()<>]+\)))*\))+(?:\((?:[^\s()<>]|(?:\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:\'\".,<>?«»“”‘’])/ig;
...
const linkify = function(text) {
  ...
  for (let match of text.matchAll(urlRE)) {
    ...
    let url = match[0];
    if (!/^[a-z][\w\-]+:/.test(url)) { url = `http://${url}`; }
...
export var chunk_text = function(text) {
  if (!text) { return []; }
  let to_prepend = [];
    ...
    if (paragraph) {
      ...
      for (let mention of paragraph.matchAll(/([\s]|^)@([a-zA-Z0-9_]*)/g)) {
        if ((mention.index > tail_start) || mention[1].length) {
          const interval = paragraph.slice(tail_start, mention.index) + mention[1];
          result.push(...linkify(interval));
        }
        result.push({type: 'mention', content: mention[2]});
        tail_start = mention.index + mention[0].length;
      }
      if (tail_start < paragraph.length) { result.push(...linkify(paragraph.slice(tail_start))); }
    }
  }
  return result;
};

export var chunk_html = function(html) {
