...
describe("puzzle feed", () =>
  it("adds", function () {
    ...
    p.addedAt({ timestamp: 1500, puzzles: [] }, 1);
    assert.isTrue(callback.calledThrice, "initial");
    assert.deepEqual(p.data, [
      { x: 1000, y: 1, isMeta: false },
      { x: 1500, y: 2, isMeta: true },
      { x: 2000, y: 3, isMeta: false },
    ]);
    ...
    assert.deepEqual(p.data, [
      ...
      { x: 2000, y: 3, isMeta: false },
      { x: 2500, y: 4, isMeta: false },
      { x: 3500, y: 5, isMeta: false },
    ]);
    callback.resetHistory();
    Session.set("currentTime", 4000);
    p.updateNow();
    p.changedAt({ timestamp: 4500, puzzles: [] }, { timestamp: 3500 }, 4);
