...
const linkify = (function() {
  ...
  const urlRE = /(?:[a-z][\w\-]+:(?:\/{1,3}|[a-z0-9%])|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}\/)(?:[^\s()<>]|\((?:[^\s()<>]|(?:\([^\s()<>]+\)))*\))+(?:\((?:[^\s()<>]|(?:\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:\'\".,<>?«»“”‘’])/i;
  ...
  // ok!
  return input => // shouldn't really ever reach here
  input.replace(re, function(text,sp,url,hashtag,username) { switch (false) {
    case (url == null): return `${sp}<a href='${url}' target='_blank'>${url}</a>`;
    case (hashtag == null): return `${sp}<a href='https://twitter.com/search?q=${encodeURIComponent(hashtag)}' target='_blank'>${hashtag}</a>`;
    case (username == null): return `${sp}<a href='https://twitter.com/${encodeURIComponent(username.slice(1))}' target='_blank'>${username}</a>`;
    default: return text;
  }
   });
})();

const htmlify = function(data) {
  const text = data.extended_tweet?.full_text ?? data.full_text ?? data.text;
  return linkify(text);
};

export default tweetToMessage = function(data) {
  if (data.retweeted_status != null) { return; } // don't report retweets
  if (data.user == null) { // weird bug we saw
