...
const DOC_NAME = name => `Notes: ${name}`;
...
const PERMISSION_LIST_FIELDS = 'permissions(role,type,emailAddress,allowFileDiscovery)';

const quote = str => `'${str.replace(/([\'\\])/g, '\\$1')}'`;

const samePerm = (p, pp) => ((p.allowFileDiscovery || true) === (pp.allowFileDiscovery || true)) && 
(p.role === pp.role) && 
(p.type === pp.type) && 
(p.type === 'anyone' ?
  true
:
  (p.emailAddress === pp.emailAddress));

const ensurePermissions = async function(drive, id) {
  ...
  const resp = (await drive.permissions.list({fileId: id, fields: PERMISSION_LIST_FIELDS})).data;
  const ps = [];
  perms.forEach(function(p) {
    // does this permission already exist?
    const exists = resp.permissions.some(pp => samePerm(p.resource, pp));
    if (!exists) {
      return ps.push(drive.permissions.create({fileId: id, ...p}));
    }});
...
const ensure = async function(drive, name, folder, settings) {
  let doc = (await drive.files.list({
...
const awaitFolder = async function(drive, name, parent) {
  let triesLeft = 5;
  while (true) {
...
const ensureFolder = async function(drive, name, parent) {
...
const awaitOrEnsureFolder = async function(drive, name, parent) {
  let res;
...
var rmrfFolder = async function(drive, id) {
  let resp = {};
  const ps = [];
  while (true) {
    // delete subfolders
    ...
    if (resp.nextPageToken == null) { break; }
  }
  while (true) {
    ...
    resp.files.forEach(item => ps.push(drive.files.delete({fileId: item.id})));
    if (resp.nextPageToken == null) { break; }
  }
  await Promise.all(ps);
...
export class Drive {
  constructor(drive) {
    this.drive = drive;
    this.rootFolder = (Promise.await(awaitOrEnsureFolder(this.drive, ROOT_FOLDER_NAME()))).id;
    this.ringhuntersFolder = (Promise.await(awaitOrEnsureFolder(this.drive, `${Meteor.settings?.public?.chatName ?? 'Ringhunters'} Uploads`, this.rootFolder))).id;
  }
  
  createPuzzle(name) {
    const {folder, permissionsPromise} = Promise.await(ensureFolder(this.drive, name, this.rootFolder));
    // is the spreadsheet already there?
    const spreadsheetP = ensure(this.drive, name, folder, spreadsheetSettings);
    const [spreadsheet, p] = Promise.await(Promise.all([spreadsheetP, permissionsPromise]));
    return {
      id: folder.id,
  ...
  findPuzzle(name) {
    const resp = (Promise.await(this.drive.files.list({
      q: `name=${quote(name)} and mimeType=${quote(GDRIVE_FOLDER_MIME_TYPE)} and ${quote(this.rootFolder)} in parents`,
      pageSize: 1
    }))).data;
    const folder = resp.files[0];
    if (folder == null) { return null; }
    // look for spreadsheet
    const spread = Promise.await(this.drive.files.list({
  ...
  listPuzzles() {
    ...
    const results = [];
    while (true) {
      resp = (Promise.await(this.drive.files.list({
        q: `mimeType=${quote(GDRIVE_FOLDER_MIME_TYPE)} and ${quote(this.rootFolder)} in parents`,
        pageSize: MAX_RESULTS,
        pageToken: resp.nextPageToken
      }))).data;
  ...
  renamePuzzle(name, id, spreadId) {
    const ps = [this.drive.files.update({
      fileId: id,
      resource: {
        name
      }
    ...
    if (spreadId != null) {
      ps.push(this.drive.files.update({
        fileId: spreadId,
        resource: {
  ...
  deletePuzzle(id) { return Promise.await(rmrfFolder(this.drive, id)); }

  // purge `rootFolder` and everything in it
  purge() { return Promise.await(rmrfFolder(this.drive, rootFolder)); }
}

// generate functions
const skip = type => () => console.warn("Skipping Google Drive operation:", type);
