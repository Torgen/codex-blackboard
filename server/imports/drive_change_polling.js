...
// Exposed for testing
export var startPageTokens = new Mongo.Collection('start_page_tokens');
startPageTokens.createIndex(({timestamp: 1}));

// Exposed for testing
// Fields:
// announced: timestamp when the existence of this file was announced in chat
export var driveFiles = new Mongo.Collection('drive_files');

// TODO: make configurable?
const POLL_INTERVAL = 60000;

const CHANGES_FIELDS = "nextPageToken,newStartPageToken,changes(changeType,removed,fileId,file(name,mimeType,parents,createdTime,modifiedTime,webViewLink))";
...
export default class DriveChangeWatcher {
  constructor(driveApi, rootDir, env = Meteor) {
    ...
    this.env = env;
    let lastToken = startPageTokens.findOne({}, {limit: 1, sort: {timestamp: -1}});
    if (!lastToken) {
      ({data: {startPageToken}} = Promise.await(this.driveApi.changes.getStartPageToken()));
      lastToken = {
        timestamp: Date.now(),
    ...
    this.timeoutHandle = this.env.setTimeout((() => this.poll()), Math.max(0, (lastToken.timestamp + POLL_INTERVAL) - Date.now()));
  }
  ...
  poll() {
    ...
    try {
      ...
      const promises = [];
      while (true) {
        ({data} = Promise.await(this.driveApi.changes.list({
          pageToken: token,
          pageSize: 1000,
        ...
        promises.push(...data.changes.map(async ({changeType, removed, fileId, file}) => {
          if (changeType !== 'file') { return; }
          if (removed) { return; }
          const {name, mimeType, parents, createdTime, modifiedTime, webViewLink} = file;
          ...
          if (createdAt > moddedAt) { moddedAt = createdAt; }
          if (parents.includes(this.rootDir)) {
            ...
            puzzleId = puzzle._id;
            if ((puzzle.spreadsheet !== fileId) && (puzzle.doc !== fileId)) { channel = `puzzles/${puzzleId}`; }
          }
          if (puzzleId != null) {
            ...
            if (update.timestamp <= moddedAt) { update.timestamp = moddedAt; }
            if ((mimeType === GDRIVE_DOC_MIME_TYPE) && (puzzle != null) && (puzzle.doc == null)) { if (update.doc == null) { update.doc = fileId; } }
          }
          if (channel != null) {
            if ((await driveFiles.rawCollection().findOne({_id: fileId}))?.announced == null) {
              return created.set(fileId, {name, mimeType, webViewLink, channel});
            }
          }
      }));
      ...
      const bulkPuzzleUpdates = (() => {
        let doc, timestamp;
        const result = [];
        for ([puzzle, {timestamp, doc}] of updates) {
      ...
      :
        Promise.resolve();
      created.forEach(function({name, mimeType, webViewLink, channel}, fileId) {
        ...
        return driveFiles.upsert(fileId,
          {$max: {announced: now}});
      });
    ...
    return this.timeoutHandle = this.env.setTimeout((() => this.poll()), POLL_INTERVAL);
  }

  stop() {
    return this.env.clearTimeout(this.timeoutHandle);
  }
}
