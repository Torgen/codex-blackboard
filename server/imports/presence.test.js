...
describe('presence', function() {
  ...
  describe('join', function() {
    ...
    it('ignores existing presence', async function() {
      ...
      await delay(200);
      return chai.assert.isUndefined(Messages.findOne({presence: 'join', nick: 'torgen'}));
    });
    ...
    it('uses nickname when no users entry', function() {
      ...
      Presence.insert({
        ...
        joined_timestamp: 8,
        clients: [{connection_id: 'test', timestamp: 9}]});
      return waitForDocument(Messages, {nick: 'torgen', presence: 'join'}, {
        system: true,
        room_name: 'general/0',
    ...
    return it('uses real name from users entry', function() {
      presence = watchPresence();
      Meteor.users.insert({
        _id: 'torgen',
        nickname: 'Torgen',
  ...
  describe('part', function() {
    ...
    it('ignores oplog room', async function() {
      ...
      Presence.remove(id);
      await delay(200);
      return chai.assert.isUndefined(Messages.findOne({presence: 'part', nick: 'torgen'}));
    });
    ...
    it('removes stale presence', async function() {
      ...
      clock.tick(240000);
      await delay(200);
      return chai.assert.isUndefined(Presence.findOne(id));
    });
    ...
    it('uses nickname when no users entry', function() {
      ...
      presence = watchPresence();
      Presence.remove(id);
      return waitForDocument(Messages, {nick: 'torgen', presence: 'part'}, {
        system: true,
        room_name: 'general/0',
  ...
  return describe('update', function() {
    it('updates unsolved puzzle', function() {
      ...
      Presence.update({nick: 'torgen', room_name:'puzzles/foo'},
        {$set: {timestamp: 15}});
      return waitForDocument(Puzzles, {_id: 'foo', solverTime: 54}, {});
  });
    ...
    it('ignores bot user', function() {
      ...
      Presence.update({nick: 'botto', room_name:'puzzles/foo'},
        {$set: {timestamp: 15}});
      return waitForDocument(Puzzles, {_id: 'foo', solverTime: 45}, {});
  });

    return it('ignores solved puzzle', async function() {
      Presence.insert({
        nick: 'torgen',
        room_name: 'puzzles/foo',
        scope: 'chat',
      ...
      Presence.update({nick: 'torgen', room_name:'puzzles/foo'},
        {$set: {timestamp: 15}});
      await delay(200);
      return chai.assert.deepInclude(Puzzles.findOne('foo'),
        {solverTime: 45});
    });
  });
});
