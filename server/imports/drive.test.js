...
describe('drive', function() {
  ...
  beforeEach(function() {
    ...
    return Meteor.settings.folder = 'Test Folder';
  });

  afterEach(function() {
    clock.restore();
    return sinon.verifyAndRestore();
  });

  it('propagates errors', function() {
    sinon.replace(batch, 'DO_BATCH_PROCESSING', false);
    files.expects('list').once().rejects({code: 400});
    return chai.assert.throws(() => new Drive(api));
  });

  const testCase = function(perms) {
    it('creates folder when batch is enabled', function() {
      ...
      perms.forEach(perm => permissions.expects('create').withArgs(sinon.match({
        fileId: 'hunt',
        resource: perm
      })).resolves({data: {}}));
      files.expects('list').withArgs(sinon.match({
    ...
    return describe('with batch disabled', function() {
      let drive = null;
      beforeEach(function() {
        sinon.replace(batch, 'DO_BATCH_PROCESSING', false);
        files.expects('list').withArgs(sinon.match({
      ...
      describe('createPuzzle', function() {
        it('creates', function() {
          ...
          return drive.createPuzzle('New Puzzle');
        });

        return it('returns existing', function() {
          files.expects('list').withArgs(sinon.match({
            q: 'name=\'New Puzzle\' and \'hunt\' in parents',
            pageSize: 1
          })).resolves({data: { files: [{
      ...
      describe('findPuzzle', function() {
        it('returns null when no puzzle', function() {
          ...
          return chai.assert.isNull(drive.findPuzzle('New Puzzle'));
        });
        
        return it('returns spreadsheet', function() {
          files.expects('list').withArgs(sinon.match({ 
            q: 'name=\'New Puzzle\' and mimeType=\'application/vnd.google-apps.folder\' and \'hunt\' in parents',
            pageSize: 1
          })).resolves({data: { files: [{
          ...
          return chai.assert.include(drive.findPuzzle('New Puzzle'), {
            id: 'newpuzzle',
            spreadId: 'newsheet'
          }
          );
      ...
      it('listPuzzles returns list', function() {
        ...
        return chai.assert.sameDeepOrderedMembers(drive.listPuzzles(), [item1, item2]);
    });
      ...
      it('renamePuzzle renames', function() {
        ...
        return drive.renamePuzzle('Old Puzzle', 'newpuzzle', 'newsheet');
      });

      return it('deletePuzzle deletes', function() {
        files.expects('list').withArgs(sinon.match({
        ...
        files.expects('delete').withArgs(sinon.match({
          fileId: 'newdoc'})).resolves({data: {}});
        files.expects('delete').withArgs(sinon.match({
          fileId: 'newpuzzle'})).resolves({data: {}});
        return drive.deletePuzzle('newpuzzle');
      });
    });
  };
  describe('with drive owner set', function() {
    beforeEach(function() {
      ...
      return Meteor.settings.drive_share_group = undefined;
    });

    return testCase(defaultPerms);
  });

  return describe('with no drive owner set', function() {
    beforeEach(() => Meteor.settings.driveowner = undefined);
    
    return testCase([EVERYONE_PERM]);
});
});
