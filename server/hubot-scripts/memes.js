...
export default scripts.memes = function(robot) {
  ...
  robot.hear(/ONE DOES NOT SIMPLY (.*)/i, msg => memegen(msg, 'mordor', 'ONE DOES NOT SIMPLY', msg.match[1]));

  robot.commands.push('bot IF YOU <text> GONNA HAVE A BAD TIME - Ski Instructor');
  robot.hear(/(IF YOU .*\s)(.* GONNA HAVE A BAD TIME)/i, msg => memegen(msg, 'ski', msg.match[1], msg.match[2]));
  ...
  robot.hear(/(IF .*), ((ARE|CAN|DO|DOES|HOW|IS|MAY|MIGHT|SHOULD|THEN|WHAT|WHEN|WHERE|WHICH|WHO|WHY|WILL|WON\'T|WOULD)[ \'N].*)/i, msg => memegen(msg, 'philosoraptor', msg.match[1], msg.match[2] + (msg.match[2].search(/\?$/)===(-1) ? '?' : '')));
  ...
  robot.hear(/WHAT IF I TOLD YOU (.*)/i, msg => memegen(msg, 'morpheus', 'WHAT IF I TOLD YOU', msg.match[1]));
  ...
  robot.hear(/(IF .*)(THAT\'D BE GREAT)/i, msg => memegen(msg, 'officespace', msg.match[1], msg.match[2]));

  robot.commands.push('bot MUCH <text> (SO|VERY) <text> - Generates Doge');
  robot.hear(/(MUCH .*) ((SO|VERY) .*)/i, msg => memegen(msg, 'doge', msg.match[1], msg.match[2]));

  robot.commands.push('bot <text> EVERYWHERE - Generates Buzz Lightyear');
  return robot.hear(/(.*)(EVERYWHERE.*)/i, msg => memegen(msg, 'buzz', msg.match[1], msg.match[2]));
};
...
const encode = s => s.toLowerCase().replace(/[-_ ?%\#/\"]/g, c => convTable[c]);

var memegen = function(msg, imageName, topText, botText) {
  if (msg.match[0].length > MaximumMemeLength.get()) {
    console.log(`Got a ${imageName} meme but it was too long`);
    return;
  }
  const url = `${memeGeneratorUrl}/${imageName}/${encode(topText)}/${encode(botText)}.jpg`;
  return msg.send(url);
};
