...
describe('memes hubot script', function() {
  ...
  it('triggers multiple memes', function() {
    ...
    const interesting = waitForDocument(Messages, {nick: 'testbot', body: /https:\/\/memegen.link\/interesting/}, {
      room_name: 'general/0',
      timestamp: 7
    }
    );
    const buzz = waitForDocument(Messages, {nick: 'testbot', body: /https:\/\/memegen.link\/buzz/}, {
      room_name: 'general/0',
      timestamp: 7
    }
    );
    const xy = waitForDocument(Messages, {nick: 'testbot', body: /https:\/\/memegen.link\/xy/}, {
      room_name: 'general/0',
      timestamp: 7
    }
    );
    const success = waitForDocument(Messages, {nick: 'testbot', body: /https:\/\/memegen.link\/success/}, {
      room_name: 'general/0',
      timestamp: 7
    }
    );
  ...
  return it('maximum lemgth applies', async function() {
    ...
    await delay(200);
    return chai.assert.isUndefined(Messages.findOne({nick: 'testbot', timestamp: 7}));
  });
});