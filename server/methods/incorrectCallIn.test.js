...
describe('incorrectCallIn', function() {
  ...
  describe('on answer', function() {
    ...
    it('fails without login', () => chai.assert.throws(() => Meteor.call('incorrectCallIn', callin)
    , Match.Error));
    ...
    describe('when logged in', function() {
      ...
      it('oplogs', () => chai.assert.lengthOf(Messages.find({type: 'puzzles', id: puzzle, stream: 'callins'}).fetch(), 1));

      it("notifies puzzle chat", () => chai.assert.lengthOf(Messages.find({room_name: `puzzles/${puzzle}`, dawn_of_time: {$ne: true}}).fetch(), 1));

      it("notifies general chat", () => chai.assert.lengthOf(Messages.find({room_name: 'general/0', dawn_of_time: {$ne: true}}).fetch(), 1));
    ...
    return describe('when not onduty', function() {
  ...
  describe('on interaction request', function() {
    ...
    describe('without response', function() {
      ...
      return describe('when logged in', function() {
        ...
        it('does not oplog', () => chai.assert.lengthOf(Messages.find({type: 'puzzles', id: puzzle, stream: 'callins'}).fetch(), 0));

        it("notifies puzzle chat", function() {
          const o = Messages.find({room_name: `puzzles/${puzzle}`, dawn_of_time: {$ne: true}}).fetch();
    ...
    return describe('with response', function() {

      it('fails without login', () => chai.assert.throws(() => Meteor.call('incorrectCallIn', callin, 'sediment')
      , Match.Error));
  ...
  return describe('on expected callback', function() {
    beforeEach(function() {
      puzzle = Puzzles.insert({
        name: 'Foo',
        canon: 'foo',
    ...
    describe('without response', function() {
      ...
      return it('fails when logged in', () => chai.assert.throws(() => callAs('incorrectCallIn', 'cjb', callin)
      , Meteor.Error));
    });
    ...
    return describe('with response', function() {
      ...
      return it('fails when logged in', () => chai.assert.throws(() => callAs('incorrectCallIn', 'cjb', callin, 'sediment')
      , Meteor.Error));
    });
  });
});
