...
const throttle = function(func, wait = 0) {
  ...
  var run = function() {
    ...
    return Meteor.setTimeout(later, wait);
  };
  return function(...a) {
    if (pending) { return; }
    [context, args] = [this, a];
    if (running) {
      ...
      running = true;
      return Meteor.setTimeout(run, 0);
    }
  };
};
...
(function() {
  ...
  const runBatch = () => Meteor.users.find({
    ...
    return Meteor.users.update(n._id, {
      $set: {
  ...
  return Meteor.users.find({
    ...
    added(id, fields) { return maybeRunBatch(); },
    // also run batch on removed: batch size might not have been big enough
    removed(id) { return maybeRunBatch(); }
  });
})();
