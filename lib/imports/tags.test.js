...
describe("getTag", function () {
  ...
  it("accepts nonmatching tags", () =>
    chai.assert.isUndefined(
      tags.getTag({ tags: { yo: { name: "Yo", value: "ho ho" } } }, "foo"),
    ));

  it("accepts matching tags", () =>
    chai.assert.equal(
      tags.getTag({ tags: { yo: { name: "Yo", value: "ho ho" } } }, "yo"),
      "ho ho",
    ));
...
describe("isStuck", function () {
  ...
  it("ignores other tags", () =>
    chai.assert.isFalse(
      tags.isStuck({ tags: { yo: { name: "Yo", value: "ho ho" } } }),
    ));

  it("ignores nonstuck status", () =>
    chai.assert.isFalse(
      tags.isStuck({
        tags: { status: { name: "Status", value: "making progress" } },
      }),
    ));

  it("matches stuck status", () =>
    chai.assert.isTrue(
      tags.isStuck({ tags: { status: { name: "Status", value: "stuck" } } }),
    ));

  it("matches verbose stuck status", () =>
    chai.assert.isTrue(
      tags.isStuck({
        tags: { status: { name: "Status", value: "Stuck to the wall" } },
      }),
    ));
});
...
describe("canonicalTags", function () {
  it("fills entries", function () {
    const pre = Date.now();
    const { foo, baz } = tags.canonicalTags(
      [
        { name: "Foo", value: "bar" },
        { name: "BaZ", value: "qux" },
      ],
    ...
    chai.assert.include(foo, {
      name: "Foo",
      value: "bar",
      touched_by: "torgen",
    });
    chai.assert.isAtLeast(foo.touched, pre);
    chai.assert.include(baz, {
      name: "BaZ",
      value: "qux",
      touched_by: "torgen",
    });
  ...
  it("preserves touched", function () {
    ...
    chai.assert.deepEqual(
      tags.canonicalTags(
        [{ name: "Foo", value: "bar", touched: pre }],
        "torgen",
      ),
      {
  ...
  it("preserves touched_by", function () {
    const { foo } = tags.canonicalTags(
      [{ name: "Foo", value: "bar", touched_by: "cscott" }],
      "torgen",
    );
    chai.assert.include(foo, {
      name: "Foo",
