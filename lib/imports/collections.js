...
export var Names = (BBCollection.names = 
  Meteor.isClient ? new Mongo.Collection('names') : null);
...
export var LastAnswer = (BBCollection.last_answer = 
  Meteor.isClient ? new Mongo.Collection('last-answer') : null);
...
export var Messages = (BBCollection.messages = new Mongo.Collection("messages"));
if (Meteor.isServer) {
...
export var LastRead = (BBCollection.lastread = new Mongo.Collection("lastread"));
if (Meteor.isServer) {
...
export var Presence = (BBCollection.presence = new Mongo.Collection('scoped_presence'));
if (Meteor.isServer) {
...
export var Calendar = (BBCollection.calendar = new Mongo.Collection('calendar'));
...
export var CalendarEvents = (BBCollection.calendar_events = new Mongo.Collection('calendar_events'));
if (Meteor.isServer) {
...
export var collection = function(type) {
...
export var pretty_collection = function(type) {
  switch (type) {
    case "oplogs": return "operation log";
    default: return type.replace(/s$/, '');
  }
};
