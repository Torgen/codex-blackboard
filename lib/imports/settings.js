...
class Setting {
  ...
  // Async on server and in methods; synchronous and reactive on client otherwise.
  get() {
    if (Meteor.isServer || Meteor.isSimulation) {
      return Settings.findOneAsync(this.canon).then(({value}) => {
        if (value != null) {
  ...
  set(value) {
    if (Meteor.isServer || Meteor.isSimulation) {
      return Meteor.callAsync("changeSetting", this.canon, value);
    } else {
      return Meteor.serializeCall("changeSetting", this.canon, value);
    }
  }
  ...
  convert(raw) {
    ...
    return Settings.upsertAsync(this.canon, {
      $setOnInsert: {
  ...
  async reset() {
    if (!Meteor.isTest) {
      throw new Error("Can only reset setting in test");
    }
    return Settings.upsertAsync(this.canon, {
      $set: {
        value: this.convert(this.default),
...
const url_matcher = Match.Where(function (url) {
  ...
  try {
    ...
    throw new Match.Error(`Could not parse ${url} as URL`);
  }
...
export var UrlSeparator = new Setting(
  "URL Separator",
  "The character used to replace spaces in the puzzle/round URL.",
  "-",
  String,
  id
);
...
export var StatsCollectionTime = new Setting(
  "Statistics Collection Time",
  "How many minutes between collections of statistics (e.g. solvers online) for projector-mode charts. 0 means don't collect.",
  0,
  Match.Integer,
  parseInt
);

export var TypingIndicatorCharacters = new Setting(
  "Typing Indicator Characters",
  'How many characters of input are necessary before the "X is typing" indicator is shown to other participants. 0 means never show it.',
  10,
  Match.Integer,
  parseInt
);
...
export var StaticJitsiMeeting = new Setting(
  ...
  "The name of the jitsi room to use for the blackboard and logistics page",
  Meteor.isServer
    ? (Meteor.settings?.jitsi?.staticRoom ||
      process.env.STATIC_JITSI_ROOM ||
      "")
    : "",
...
export async function ensureAll() {
  if (!ensurePromise) {
    const ensures = Object.values(all_settings).map((x) => x.ensure());
    ensurePromise = Promise.all(ensures);
  }
  await ensurePromise;
}

if (Meteor.isServer) {
  Meteor.startup(ensureAll);
}

export async function resetAll() {
  const resets = Object.values(all_settings).map((x) => x.reset());
  await Promise.all(resets);
}

Meteor.methods({
  async changeSetting(setting_name, raw_value) {
    ...
    return (
      0 <
      (await Settings.updateAsync(canonical_name, {
        $set: {
