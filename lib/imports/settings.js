...
class Setting {
  constructor(name, description, default1, matcher, parser) {
    ...
    if (Meteor.isServer) { this.ensure(); }
    Object.freeze(this);
  }
  ...
  get() {
    try {
      ...
      if (value != null) { return this.convert(value); }
    } catch (error) {
      return console.warn(`get setting ${this.name} failed`, error);
    }
  }

  // Requires login. On server, from non-method code, use impersonating() to
  // pretend to be a user.
  set(value) { return Meteor.call('changeSetting', this.canon, value); }
  ...
  convert(raw) {
    if (Match.test(raw, this.matcher)) { return raw; }
    check(raw, String);
    ...
    return Settings.upsert(this.canon, {
      $setOnInsert: {
...
const parse_boolean = function(x) {
  switch (x) {
    case 'true': return true;
    case 'false': return false;
    default: throw new Match.Error(`Bad boolean string ${x}`);
  }
};
...
export var StaticJitsiMeeting = new Setting(
  ...
  Meteor.isServer ? (Meteor.settings?.jitsi?.staticRoom ?? process.env.STATIC_JITSI_ROOM ?? '') : '',
...
Meteor.methods({
  changeSetting(setting_name, raw_value) {
    ...
    return 0 < Settings.update(canonical_name, { $set: {
