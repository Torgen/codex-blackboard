...
describe 'fileType', ->
  ...
  it 'capitalizes Google ', ->
    chai.assert.equal fileType('application/vnd.google-apps.jam'), 'Google Jam'

  it 'blats unknown', ->
    chai.assert.equal fileType('application/octet-stream'), 'application/octet-stream File'