 ...
 */
export var NumberInRange = args => Match.Where(function(x) {
  check(x, Number);
  if (args.min != null) {
    if (x < args.min) { return false; }
  }
  if (args.max != null) {
    if (x > args.max) { return false; }
  }
  return true;
});

export var StringWithLength = args => Match.Where(function(x) {
  check(x, String);
  check(x.length, NumberInRange(args));
  return true;
});

export var ArrayWithLength = (matcher, args) => Match.Where(function(x) {
  check(x, [matcher]);
  check(x.length, NumberInRange(args));
  return true;
});
...
export var ArrayMembers = arr => Match.Where(function(x) {
...
export var IdOrObject = Match.OneOf(NonEmptyString, Match.Where(o => (typeof o === 'object') && ((check(o._id, NonEmptyString)) || true))
);

// This is like Match.ObjectIncluding, but we don't require `o` to be
// a plain object
export var ObjectWith = pattern => Match.Where(function(o) {
