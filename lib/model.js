...
(function () {
  ...
  function setAnswerInternal(call, puzzle, answer, backsolve, provided) {
  ...
  function recentSolverTime(id) {
    ...
    Puzzles.update(id, { $inc: { solverTime } });
  }
  ...
  Meteor.methods({
    ...
    correctCallIn(id, response) {
      check(this.userId, NonEmptyString);
      check(id, NonEmptyString);
      const { callin, msg, puzzle, solved } = runInTransaction(
        () => {
          ...
          if (callin.target_type === "puzzles") {
            puzzle = Puzzles.findOne(callin.target, {
              fields: { answers: 1, feedsInto: 1, name: 1, "tags.answer": 1 },
            });
          }
          if (callin.callin_type === callin_types.PARTIAL_ANSWER) {
            ...
            if (response) {
              if (puzzle.answers) {
                ...
                Puzzles.update({ _id: puzzle._id }, updateDoc);
                Object.assign(msg, {
    ...
    setTag(args) {
      ...
      const result = 0 < collection(args.type).update(id, updateDoc);
      if (result && args.type === "puzzles") {
        if (canonical(args.name).startsWith("meta_")) {
          const meta = Puzzles.findOne(id, { fields: { name: 1, puzzles: 1 } });
          if (meta) {
            meta.puzzles?.forEach((feeder) => {
          ...
          const meta = Puzzles.findOne(id, { fields: { name: 1, puzzles: 1 } });
          if (meta) {
            meta.puzzles?.forEach((feeder_id) => {
              const feeder = Puzzles.findOne(feeder_id, {
                fields: { tags: 1 },
              });
              if (!feeder) {
                return;
    ...
    setAnswer(args) {
      ...
      const result = runInTransaction(
        () => {
          const puzzle = Puzzles.findOne(id, { fields: { "tags.answer": 1 } });
          return setAnswerInternal(
            this,
            puzzle,
            args.answer,
            args.backsolve,
    ...
    deletePartialAnswer(_id, answer) {
      check(this.userId, NonEmptyString);
      check(_id, String);
      check(answer, String);
      const changed = Puzzles.update({_id, solved: null, answers: answer}, {
        $pull: {
          answers: answer
        },
        $set: {
      ...
      if (changed) {
        oplog(`Deleted partial answer ${answer} for`, "puzzles", _id, this.userId);
      }
    },
