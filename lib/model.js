...
(function () {
  ...
  function recentSolverTime(id) {
    ...
    Presence.find({
      ...
      if (since < (PRESENCE_KEEPALIVE_MINUTES * 60 + 10) * 1000) {
        ...
        return (acc += since - PRESENCE_KEEPALIVE_MINUTES * 30 * 1000);
      }
    });
    return acc;
  }
  ...
  Meteor.methods({
    ...
    // Response is forbibben for answers, mandatory boolean (meaning is this
        ...
        if (response) {
          if (puzzle.answers) {
            ...
            const updateDoc = {
              ...
              $inc: {
                solverTime: recentSolverTime(puzzle._id),
              },
              $push: {
    ...
    setAnswer(args) {
      ...
      const solverTime = recentSolverTime(id);
